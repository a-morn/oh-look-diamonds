...
const scrollBar = {
	bar: null,
	slider: null,
	cont: new createjs.Container() // eslint-disable-line no-undef
};
...
export function init(data) {
	...
	lightningColor = data.lightningColor;
	bg = data.bg;
	stage = data.stage;
	queue = data.queue;
	...
	rocketSong = createjs.Sound.play('palladiumAlloySong'); // eslint-disable-line no-undef
	rocketSong.stop();		
	...
	Containers.onlooker = new createjs.Container(); // eslint-disable-line no-undef
	...
	polygonLine = new createjs.Shape(); // eslint-disable-line no-undef
	Containers.collisionCheckDebug.addChild(polygonLine);
	
	{
		const	diamondSound = createjs.Sound.play('diamondSound'); // eslint-disable-line no-undef
		diamondSound.volume = 0.2;
		diamondSound.stop();
	}
	
	{
		squawkSound = createjs.Sound.play(name); // eslint-disable-line no-undef
		squawkSound.volume=0.15;
		squawkSound.stop();
	}
	gameView = new createjs.Container(); // eslint-disable-line no-undef
...
export function SetObjectEventListeners(turnOn) {
	...
	for (var i = levelDesignConts.length - 1; i >= 0; i--) {
		...
		for (var j = container.children.length - 1; j >= 0; j--) {
			let kid = container.getChildAt(j);
			if (turnOn) {
				setupObjEvents(kid);
			} else {
...
export function CutCopy(isCopy) {
	...
	for (var i = Containers.select.children.length - 1; i >= 0; i--) {
		let kid = Containers.select.children[i];
...
export function SelectWithBox() {
	...
	for (var i = levelDesignConts.length - 1; i >= 0; i--) {
		var container = levelDesignConts[i];
		for (var j = container.children.length - 1; j >= 0; j--) {
			const kid = container.getChildAt(j);
...
export function CreateThunderCloud() {
	...
	if (createjs.Ticker.paused) { // eslint-disable-line no-undef
		setupObjEvents(kid);
	}
	stage.update();
}
...
export function gotoGameView(stage) {
	...
	gameListener = createjs.Ticker.on('tick', update, // eslint-disable-line no-undef
		this);
...
function updateWorldContainer() {
	...
	if ((!createjs.Tween.hasActiveTweens(gameView) && !createjs.Tween.hasActiveTweens(bg) && !createjs.Tween.hasActiveTweens(Containers.star)) && !CatzRocket.isCrashed) { // eslint-disable-line no-undef
		if (!zoomOut && CatzRocket.catzRocketContainer.y < 0 && CatzRocket.catzState === CatzRocket.catzStateEnum.Normal) {
			createjs.Tween.get(gameView) // eslint-disable-line no-undef
				.to({
					scaleX: zoomOutLimit,
					scaleY: zoomOutLimit
				}, zoomDuration, createjs.Ease.linear) // eslint-disable-line no-undef
				.call(function() {
					zoomOut = true;
				});

			createjs.Tween.get(bg) // eslint-disable-line no-undef
				.to({
					scaleX: 1,
					scaleY: zoomOutLimit
				}, zoomDuration, createjs.Ease.linear); // eslint-disable-line no-undef

			createjs.Tween.get(Containers.star) // eslint-disable-line no-undef
				.to({
					scaleX: zoomOutLimit,
					scaleY: zoomOutLimit
				}, zoomDuration, createjs.Ease.linear); // eslint-disable-line no-undef
...
function spawnThunderCloud(xPos, yPos) {
	createjs.Sound.play('thunder'); // eslint-disable-line no-undef
	...
	cloud.x = xPos;
	cloud.y = yPos;
	cloud.filters = [new createjs.ColorFilter(0.3, 0.3, 0.3, 1, 0, 0, 55, 0)]; // eslint-disable-line no-undef
...
function updateThunderClouds() {
	for (var i = 0, arrayLength = Containers.thunder.children.length; i < arrayLength; i++) {
		...
		if (!kid.hasFired && !CatzRocket.isHit && CatzRocket.catzRocketContainer.x < (kid.x + rect.width * kid.scaleX) && CatzRocket.catzRocketContainer.x >
			...
			var spotX = 0;
			var spotY = 0;
			for (let j = 0, max = Containers.attackBird.children.length; j < max; j++) {
				var bird = Containers.attackBird.children[j];
			...
			var lightning = new createjs.Shape(); // eslint-disable-line no-undef
			...
			createjs.Tween.get(lightning).to({ // eslint-disable-line no-undef
				alpha: 0
			}, 300);
			createjs.Sound.play('lightningBolt'); // eslint-disable-line no-undef
			createjs.Tween.get(gameView) // eslint-disable-line no-undef
				.to({
					x: -50,
					y: 20
...
function generateTrack() {
	...
	if (gameStats.Difficulty >= 0) {
		for (let j = 0,
			...
			var element = $.extend(true, [], trackParts[tracks[currentLevel][currentTrack][j].difficulty][tracks[currentLevel][currentTrack][j].name]); // eslint-disable-line no-undef
			for (let i = 0,
...
function updateOnlookers(event) {
	...
	function addOnlooker(onlooker) {
		var oCont = new createjs.Container(); // eslint-disable-line no-undef
...
function updatePointer() {
	$('.progress-bar').css('width', CatzRocket.diamondFuel * 10 + '%'); // eslint-disable-line no-undef
	if (CatzRocket.diamondFuel < 2) {
		if (fuelBlinkTimer > 10) {
			$('.progress-bar').toggleClass('background-red'); // eslint-disable-line no-undef
			fuelBlinkTimer = 0;
		}
		fuelBlinkTimer++;
	}
	if (CatzRocket.diamondFuel >= 2)
		$('.progress-bar').removeClass('background-red'); // eslint-disable-line no-undef
	...
	for (var i = 0, arrayLength = Containers.diamond.children.length; i < arrayLength; i++) {
		...
		if (kid.x <= -100) {
			...
			if (overlapCheckCircle(kid.x, kid.y, 40)) {
				...
				arrayLength -= 1;
				i -= 1;
				var instance = createjs.Sound.play('diamondSound'); // eslint-disable-line no-undef
...
function spawnAttackBird(type, x, y) {
	var attackBird = new AttackBird(new createjs.SpriteSheet(SpriteSheetData.enemybirds), type); // eslint-disable-line no-undef
	attackBird.x = x;
	attackBird.y = y;
	if (type === 'duck') {
...
function updateVertices() {
	...
	if (CatzRocket.hasFrenzy()) {
		x = CatzRocket.catzRocketContainer.x + 70 * c - 13 * s;
		y = CatzRocket.catzRocketContainer.y + 13 * c + 70 * s;
		h = (newBounds.height / 2) + 5;
		w = (newBounds.width / 2) + 10;
	} else {
		x = CatzRocket.catzRocketContainer.x - 10 * c - 13 * s;
		y = CatzRocket.catzRocketContainer.y + 13 * c - 10 * s;
	...
	if (CatzRocket.isWounded) {
		x = CatzRocket.catzRocketContainer.x - 55 * c + 3 * s;
		y = CatzRocket.catzRocketContainer.y - 3 * c - 55 * s;
	} else {
		x = CatzRocket.catzRocketContainer.x - 5 * c + 3 * s;
		y = CatzRocket.catzRocketContainer.y - 3 * c - 5 * s;
	}
	h = (CatzRocket.catzBounds.height / 2);
	w = (CatzRocket.catzBounds.width / 2);
	...
	if (CatzRocket.hasFrenzy()) {
		x = CatzRocket.catzRocketContainer.x + 55 * c - 13 * s;
		y = CatzRocket.catzRocketContainer.y + 13 * c + 55 * s;
	} else {
		x = CatzRocket.catzRocketContainer.x - 40 * c - 13 * s;
		y = CatzRocket.catzRocketContainer.y + 13 * c - 40 * s;
	}
...
function collisionCheck(bird) {
	...
	if (Math.abs(bird.x - CatzRocket.catzRocketContainer.x) < 200 && Math.abs(bird.y - CatzRocket.catzRocketContainer.y) < 150) {
		...
		for (i = 0, max = polygonVertices.length; i < max; i++) {
...
function collisionResolve(bird, normX, normY, normDist, isGround) {
	if (isGround || CatzRocket.canCollide()) {
		...
		var name = 'klonk' + rand;
		var instance = createjs.Sound.play(name); // eslint-disable-line no-undef
		instance.volume = 0.15;
		if (squawkSound.playState !== createjs.Sound.PLAY_SUCCEEDED) { // eslint-disable-line no-undef
...
function drawCollisionModels() {
	polygonLine.graphics = new createjs.Graphics(); // eslint-disable-line no-undef
	polygonLine.x = 0;
	polygonLine.y = 0;
	...
	for (i = 0, max = catzVertices.length; i < max; i++) {
		polygonLine.graphics.setStrokeStyle(2, 1);
	...
	for (i = 0, max = flameVertices.length; i < max; i++) {
		polygonLine.graphics.setStrokeStyle(2, 1);
...
function catzFellOfRocket() {
	...
	createjs.Tween.removeTweens(CatzRocket.rocket); // eslint-disable-line no-undef
	createjs.Tween.get(CatzRocket.rocket).to({ // eslint-disable-line no-undef
		x: 800
	}, 800);
}
...
function crash() {
	var instance = createjs.Sound.play('catzRocketCrash'); // eslint-disable-line no-undef
	instance.volume = 0.5;
