...
export const rocketSnake = new createjs.Container(); //eslint-disable-line no-undef
...
export function init(data, queue) {
	...
	catzRocketContainer = new createjs.Container(); //eslint-disable-line no-undef
...
{x:190, y:200, regX:40, regY:-37, alpha:0});															
	...
	for(let i = 0, snakeAmt = 11; i < snakeAmt; i++){
		var shape = new createjs.Shape(); //eslint-disable-line no-undef
		var x = 260-i*5;
		var r = 9;
	...
	snakeLine = new createjs.Shape(); //eslint-disable-line no-undef
	rocketSound = createjs.Sound.play('rocketSound'); // eslint-disable-line no-undef
...
export function update(grav, wind, event) {
	...
	if (isWounded && !createjs.Tween.hasActiveTweens(catz)) { // eslint-disable-line no-undef
		catz.x = -50;
	} else if (!isWounded &&
		!createjs.Tween.hasActiveTweens(catz)) { // eslint-disable-line no-undef
		catz.x = 0;
	}
...
export function playSecondDownloopSound() {
	rocketSound.stop();
	rocketSound = createjs.Sound.play(rocketSounds[ // eslint-disable-line no-undef
...
export function getHit(isInstaGib) {
	if ((invincibilityCounter <= 0 || isInstaGib) && !hasFrenzy() && !isHit) {
		var instance = createjs.Sound.play('catzScream2'); // eslint-disable-line no-undef
		instance.volume = 0.5;
...
export function reset() {
	createjs.Tween.removeTweens(catzRocketContainer); // eslint-disable-line no-undef
	frenzyReady = false;
	frenzyTimer = 0;
	frenzyCount = 0;
	changeState(catzStateEnum.Normal);
...
function updateBase(gravWindSum, event, canChangeToTerminal, fellOff, rotate) {
	...
	if (rotate && !createjs.Tween.hasActiveTweens(catzRocketContainer)) { // eslint-disable-line no-undef
...
function updateRocketSnake() {
	const arrayLength = rocketSnake.children.length;
	for (let i = arrayLength - 1; i > 0; i--) {
		const kid = rocketSnake.children[i];
	...
	snakeLine.graphics = new createjs.Graphics(); //eslint-disable-line no-undef
	snakeLine.x = 260;
	snakeLine.y = 200;
	for (let i = arrayLength - 1; i > 0; i--) {
...
function changeState(state) {
	...
	if (state !== catzStateEnum.SlammerReady &&
		...
		if (rocketSounds[state] !== null) {
			rocketSound = createjs.Sound.play(rocketSounds[state]); // eslint-disable-line no-undef
...
function updateUploop(grav, wind, event) {
	...
	if(catzRocketContainer.rotation < -60) {
		...
		createjs.Tween.removeTweens(catzRocketContainer);  // eslint-disable-line no-undef
		createjs.Tween.get(catzRocketContainer) // eslint-disable-line no-undef
			.to({
				rotation: -270
			}, 1000)
			.to({
...
function updateSecondUploop(grav, wind, event) {
	...
	if (!createjs.Tween.hasActiveTweens(catzRocketContainer)) { // eslint-disable-line no-undef
		catzRocketContainer.rotation = Math.atan(catzVelocity / 40) * 360 / 3.14;
	}
	if (catzRocketContainer.rotation < -60) {
