...
let timmy = null;
let priest = null;
let diamondHill = null;
let subtractedDiamond = null;
...
export function init(data) {
	...
	subtractedDiamondCont = new createjs.Container();
	houseView.y = 1500;
	bg.y = 0;
...
export function gotoHouseView() {
	...
	timmy.alpha = 0;
	priest.alpha = 0;        
	cricketsSound = createjs.Sound.play("crickets",{loop:-1});
...
function goDown(){		
	...
	createjs.Tween
		.get(houseView)
		.to({y:0}, 4000, createjs.Ease.quadInOut)
		.call(load)
		.call(hoboWalk);
	createjs.Tween
		.get(bg)
		.to({y:-1200}, 4000, createjs.Ease.quadInOut);
	createjs.Tween
		.get(Containers.star)
		.to({y:0}, 4000, createjs.Ease.quadInOut);				
}	
...
function buildingAnimation (houseGraphic){		        
	...
	createjs.Tween.get(houseView, {loop:true})
		.to({x:-5, y:5},50)
		.to({x:5, y:-5},50);
}
